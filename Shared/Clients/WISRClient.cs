...
namespace S0WISRXX.Shared;

public class WISRClient : IDisposable
{
    ...
    public WISRClient(IUtilityLogger logger, string baseUrl, string tenantId, string clientId, string clientSecret, string[] scopes)
    {
        _baseUrl = baseUrl;
        _tenantId = tenantId;
        _clientId = clientId;
        ...
        _logger.LogInformation($"#####------------------ Initializing WISR CommonServices API ------------------#####");
        var apiToken = _sharedBaseClass.RequestAzureADTokenUsingApplicationIdentityWithExpiry(_tenantId, _clientId, _clientSecret, _scopes, "WISR CommonServices API").Result;
        _authTokenExpiry = DateTime.Parse(apiToken.Item2);
        _client.Timeout = TimeSpan.FromMinutes(5);
        _client.BaseAddress = new Uri(_baseUrl);
        _client.DefaultRequestHeaders.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", apiToken.Item1);
